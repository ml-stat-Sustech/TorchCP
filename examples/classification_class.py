...
def run_experiment(score_function, model_name, alpha, cal_logits,cal_labels, test_logits, test_labels, randomized, predictor):
    ...
    print("-" * 80)
    coverage = metrics('coverage_rate')(prediction_sets, test_labels)
    size = metrics('average_size')(prediction_sets, test_labels)
    covgap = metrics('CovGap')(prediction_sets, test_labels, alpha, num_classes)
    return coverage, size, covgap
...
if __name__ == '__main__':
    ...
    df = pd.DataFrame(columns=['Coverage', 'Size', 'CovGap'])
    for seed in range(10):
        ...
        for score_function in scoring_methods:
            predictor_methods = [SplitPredictor(score_function, model), 
                             ClassWisePredictor(score_function, model),
                             ClusteredPredictor(score_function, model)]
            for predictor in predictor_methods:
                coverage, size, covgap = run_experiment(
                ...
                df.loc[len(df)] = [coverage, size, covgap]
    print(df)
