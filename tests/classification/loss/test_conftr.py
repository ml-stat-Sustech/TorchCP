...
def conftr_instance():
    ...
    epsilon = 1e-4
    return ConfTr(predictor, alpha, fraction, True, epsilon, loss_type, target_size, loss_transform)
...
def test_invalid_fraction():
    with pytest.raises(ValueError):
        ConfTr(Predictor(THR()), 0.05, 0)


def test_invalid_loss_type():
    with pytest.raises(ValueError):
        ConfTr(Predictor(THR()), 0.05, 0.2, loss_type="invalid")


def test_invalid_target_size():
    with pytest.raises(ValueError):
        ConfTr(Predictor(THR()), 0.05, 0.2, target_size=2)


def test_invalid_loss_transform():
    with pytest.raises(ValueError):
        ConfTr(Predictor(THR()), 0.05, 0.2, loss_transform="invalid")


def test_invalid_epsilon():
    with pytest.raises(ValueError):
        ConfTr(Predictor(THR()), 0.05, 0.2, epsilon=0)
...
def test_loss_transform_abs():
    conftr = ConfTr(Predictor(THR()), 0.05, 0.2, loss_transform="abs")
    assert conftr.transform == torch.abs


def test_loss_transform_log():
    conftr = ConfTr(Predictor(THR()), 0.05, 0.2, loss_transform="log")
